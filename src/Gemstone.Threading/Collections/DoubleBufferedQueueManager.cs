...
public class DoubleBufferedQueueProducer<T> : IDisposable
{
    ...
    // Fields
    private readonly DoubleBufferedQueueManager<T> m_manager;
...
public class DoubleBufferedQueueManager<T>
{
    ...
    /// </summary>
    public bool ItemsLeft => m_itemsLeft;
    ...
    public IList<T> Dequeue()
    {
        lock (m_queuesLock)
        {
            ...
            {
                if (queue.TryDequeue(out IList<T> dequeuedItems) <= 0)
                {
    ...
    {
        m_itemHandler?.Invoke();

        if (m_itemsLeft)
            m_itemHandlingOperation?.RunAsync();
    }
    ...
    // Static Fields
    private static readonly IList<T> s_emptyList = Array.Empty<T>();
